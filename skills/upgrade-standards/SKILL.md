...
6. Apply prerequisites and coupled changes together, including shared configs and dependent applications. Keep Oxlint/Oxfmt as the lint/format path and verify natural sorting through Oxlint's JavaScript-plugin support; do not add an ESLint runner. When the approved lint upgrade includes anti-slop, install and invoke its official skill using [references/integrations.md](references/integrations.md#anti-slop). Install or verify Codegraph, Plannotator, and selected upstream skills where applicable. Read that reference for integration routing.

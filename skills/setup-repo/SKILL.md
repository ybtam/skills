...
2. Identify the project skills that fit the inspected target, including [install-anti-slop](references/integrations.md#anti-slop) for JavaScript/TypeScript projects using or adopting Oxlint. Install and verify only owner-selected skills where they are available and authorized. Invoke anti-slop to configure lint only when applying the approved setup in step 8. If a supporting skill is unavailable, continue with the bundled references rather than creating a dependency on a sibling skill.

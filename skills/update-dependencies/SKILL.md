...
2. Identify outdated, deprecated, incompatible, and coupled packages. For every major update, consult the package's official migration guide and release notes before editing. When updating Oxlint or React Compiler tooling in a React project, read [React Compiler support](references/integrations.md#react-compiler) for rule migration and framework compatibility. Propose deprecated replacements explicitly; do not treat a broad latest-version command as a migration plan. Report runtime incompatibility and agree a project-scoped resolution before changing runtime configuration.
...
4. Update the correct manifest and lockfile with the repository's existing package manager. Update code/config only where required by official migration guidance. For existing anti-slop installations, upgrade `oxlint` and `@oxlint/plugins` together at the same exact version and verify the vendored plugin using [references/integrations.md](references/integrations.md#anti-slop). Preserve compatibility pins and project exceptions unless the approved plan changes them.

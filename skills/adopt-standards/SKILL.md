...
7. Install or configure selected upstream prerequisites explicitly. When approved lint adoption includes anti-slop, install and invoke its official skill using [references/integrations.md](references/integrations.md#anti-slop). Read that reference when adding Matt Pocock or stack skills, Codegraph, Plannotator, agent settings, or MCP integrations. Initialize Codegraph when source exists and verify its query; verify the Plannotator review operation without overwriting existing configuration.
